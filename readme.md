...
* **Servidor (`server.go`)**: Gerencia conexões simultâneas usando *goroutines*, processa comandos baseados em um protocolo de texto, realiza validações de entrada e executa os cálculos.
...
*  **Tratamento de Tipos**: Conversões numéricas protegidas (`strconv.Atoi`) evitam que falhas de formato derrubem o servidor.
...
*  **Multicliente**: Arquitetura concorrente baseada em *goroutines* com `net.Listen` e `net.Accept`.
